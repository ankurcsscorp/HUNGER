...
let getHotelCard = singleHotelData => {
    ...
    return `<div class="col-xs-12 col-md-6 col-lg-4 col-xl-3" id="mobileManage">
      <div class="card">
...
function searchResult() {
    let inpValue = document.getElementById("SearchInput");
    let searchMatchingHotels = allHotelsDataCopy.filter(hotel => hotel.tags.toString().toUpperCase().indexOf(inpValue.value.toUpperCase()) > -1)
    mainRow.innerHTML = generateView(searchMatchingHotels).join('')
    errorMessageElement.innerText = (searchMatchingHotels == 0) ? `No hotels Found for ${inpValue.value}` : '';
}
...
selectFilter.addEventListener("click", e=> {
    ...
    errorMessageElement.innerText=(filteredHotels.length==0) ? `No Restro belongs to ${e.target.innerText}`: '';
})

// Mark favourate - red color if item is pushed into localstorage and mark white if item is removed from localstorage
